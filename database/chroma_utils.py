#chroma_utils.py: preprocess, chunk_text, normalize_vector
...
def normalize_vector(vec):
    ...
    Utile pour le cosine similarity et la recherche dans ChromaDB.
    """
...
def get_embedding(text: str, model: str = "all-minilm", max_words: int = 500):
    """
    Génère l'embedding via Ollama et le normalise.
    Tronque le texte si le nombre de mots dépasse max_words pour éviter les erreurs.
    """
    try:
        words = text.split()
...
# #version KIRIL : database/chroma_utils.py
# # chroma_utils.py
# import numpy as np
# from ollama._client import Client as OllamaClient
